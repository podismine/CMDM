...
save_data_norm = np.zeros((len(norm_dataset.ct_feas), 62))
save_data_asd = np.zeros((len(abide_dataset.ct_feas), 62))
save_data_mci = np.zeros((len(mci_dataset.ct_feas), 62))
save_data_ad = np.zeros((len(ad_dataset.ct_feas), 62))
...
for roi in range(62):
    ...
    pred_data_norm[:,roi] = m.data[m.data['group']==0]['GAMLSS_pred'].values  #pred#.reshape(-1)
    pred_data_asd[:,roi] = m.data[m.data['group']==1]['GAMLSS_pred'].values  #pred#.reshape(-1)
    pred_data_mci[:,roi] = m.data[m.data['group']==2]['GAMLSS_pred'].values  #pred#.reshape(-1)
    pred_data_ad[:,roi] = m.data[m.data['group']==3]['GAMLSS_pred'].values  #pred#.reshape(-1)

    save_data_norm[:,roi] = m.data[m.data['group']==0]['GAMLSS_z'].values  #pred#.reshape(-1)
    save_data_asd[:,roi] = m.data[m.data['group']==1]['GAMLSS_z'].values  #pred#.reshape(-1)
    save_data_mci[:,roi] = m.data[m.data['group']==2]['GAMLSS_z'].values  #pred#.reshape(-1)
    save_data_ad[:,roi] = m.data[m.data['group']==3]['GAMLSS_z'].values  #pred#.reshape(-1)

np.savez("gamlss_norm.npz",  asd = save_data_asd, mci=save_data_mci, ad=save_data_ad, norm=save_data_norm,\
...
print(f"GAMLSS-fid-norm: {np.mean([f for f in fid_errors_norm if f == f]):.4f} fid-asd: {np.mean([f for f in fid_errors_asd if f == f]):.4f} fid-mci: {np.mean([f for f in fid_errors_mci if f == f]):.4f} fid-ad: {np.mean([f for f in fid_errors_ad if f == f]):.4f}")