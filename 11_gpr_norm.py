...
for roi in range(62):
    ...
    pred_data_norm[:,roi] = m.data[m.data['group']==0]['GP_pred'].values  #pred#.reshape(-1)
    pred_data_asd[:,roi] = m.data[m.data['group']==1]['GP_pred'].values  #pred#.reshape(-1)
    pred_data_mci[:,roi] = m.data[m.data['group']==2]['GP_pred'].values  #pred#.reshape(-1)
    pred_data_ad[:,roi] = m.data[m.data['group']==3]['GP_pred'].values  #pred#.reshape(-1)

    save_data_norm[:,roi] = m.data[m.data['group']==0]['GP_z'].values  #pred#.reshape(-1)
    save_data_asd[:,roi] = m.data[m.data['group']==1]['GP_z'].values  #pred#.reshape(-1)
    save_data_mci[:,roi] = m.data[m.data['group']==2]['GP_z'].values  #pred#.reshape(-1)
    save_data_ad[:,roi] = m.data[m.data['group']==3]['GP_z'].values  #pred#.reshape(-1)

np.savez("gpr_norm.npz",  asd = save_data_asd, mci=save_data_mci, ad=save_data_ad, norm=save_data_norm,\
