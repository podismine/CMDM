...
np.savez("cvae_norm.npz",asd=pred_data_asd_err, mci=pred_data_mci_err, ad=pred_data_ad_err, norm=pred_data_norm,\
...
print(f"ACVAE-fid-norm: {np.mean([f for f in fid_errors_norm if f == f]):.4f} fid-asd: {np.mean([f for f in fid_errors_asd if f == f]):.4f} fid-mci: {np.mean([f for f in fid_errors_mci if f == f]):.4f} fid-ad: {np.mean([f for f in fid_errors_ad if f == f]):.4f}")