...
for epoch_counter in range(resume_epoch, resume_epoch+10000):
    ...
    if epoch_counter % 1000 == 0:
        torch.save({'model':model.state_dict(),'ema_model':ema_model.state_dict(), }, os.path.join(f"{checkpoint_dir}/{epoch_counter}_model.pth"))
    if best_train_loss > total_train_loss:
        best_train_loss = total_train_loss
        if epoch_counter > 1000:
            torch.save({'model':model.state_dict(),'ema_model':ema_model.state_dict(), }, os.path.join(f"{checkpoint_dir}/best_model.pth"))

torch.save({
    'model':model.state_dict(),
    'ema_model':ema_model.state_dict(),
    }, os.path.join(f"{checkpoint_dir}/last_model.pth"))
