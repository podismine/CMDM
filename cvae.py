...
def compute_ll(x, x_recon):
    #print(x.shape, x_recon.shape);exit()
...
class cVAE(nn.Module):
    ...
    def pred_recon_tensor(self, x,c, test_latent, test_var, DEVICE):
        ...
        c = torch.LongTensor(c).to(DEVICE)
        with torch.no_grad():
            _, x_pred = self.decode(torch.from_numpy(test_latent),c)
            # _, x_pred = self.decode(torch.from_numpy(test_latent),c).loc.cpu().detach().numpy()
        return x_pred.cpu().detach().numpy()
